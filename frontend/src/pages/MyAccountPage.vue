<template>
  <div class="dashboard-page">
    <div class="dashboard-top">
      <div class="text">
        <div class="date">Monday, October 20</div>
        <div class="welcome-message">Hello, User!</div>
        <div class="dashboard-description">
          Here is a quick look at your active and completed enrollments.
          You can also view a snapshot of your previous purchases. Cris's testing!!!
        </div>
      </div>

      <div class="image">
        <img src="../assets/owpart.png"/>
      </div>
    </div>

    <div class="dashboard-bottom">
      <div class="dashboard-left">
        <div class="active-enrollments">
          <div class="divider"></div>
        </div>

        <div class="instructor-slides">
          <div class="divider"></div>
        </div>
      </div>
      <div class="dashboard-right">
        <div class="messages">
          <div class="divider"></div>
        </div>
        <div class="purchase-history">
          <div class="divider"></div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
  .dashboard-page {
    display: grid;
    grid-template-rows: auto 1fr;
    row-gap: 32px;
    justify-content: center;
    align-items: top;
  }

  .dashboard-top {
    grid-row: 1;
    display: grid;
    grid-template-columns: 508px 508px;
    margin-top: 32px;

    /* new: make top of dashboard have a grey box UNDER all assests */
    background-color: #F2F1F2;   /* ← gray */
    border-radius: 1rem;
    padding: 24px;
    column-gap: 32px;
  }

  .text {
    grid-column: 1;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: left;
    height: 240px;
    font-family: Myriad Pro;
    color: #034750;
  }

  .image {
    grid-column: 2;
  }

  .image > img {
    width: 100%;
    height: 100%;
    object-fit: scale-down;            /* fills + crops */
    object-position: center;      /* change to 'top'/'50% 30%' to shift the crop */
    display: block;
  }

  .date {
    font-size: 20px;
    font-weight: 600;
    color: #707070;
    margin-top: 24px;
    margin-bottom: 32px;
    margin-left: 24px;
  }

  .welcome-message {
    font-size: 56px;
    font-weight: 700;
    color: #00A5B5;
    margin-bottom: 16px;
    margin-left: 24px;
  }

  .dashboard-description {
    width: 395px;
    height: 71px;
    font-size: 19px;
    font-weight: 400;
    color: #747474;
    margin-bottom: 33px;
    margin-left: 24px;
  }

  .dashboard-bottom {
    grid-row: 2;
    display: grid;
    grid-template-columns: 700px 300px;
    column-gap: 16rem;
    margin-bottom: 48px;
  }

  .dashboard-left {
    grid-column: 1;
    display: flex;
    flex-direction: column;
    gap: 16px;
  }

  .divider {
    width: 100%;
    height: 0px;
    margin-top: 68px;
    border: 1px, solid, #FFFFFF;
  }

  .active-enrollments {
    height: 450px;
    border-radius: 14rem;
    display: flex;
    justify-content: center;
    align-items: top;
    background-color: #F2F1F2;
  }

  .instructor-slides {
    height: 250px;
    border-radius: 14rem;
    background-color: #F2F1F2;
  }

  .dashboard-right {
    grid-column: 2;
    display: flex;
    flex-direction: column;
    gap: 16px;
  }

  .messages {
    height: 240px;
    border-radius: 14rem;
    background-color: #F2F1F2;
  }

  .purchase-history {
    height: 460px;
    border-radius: 14rem;
    background-color: #F2F1F2;
  }

</style>
